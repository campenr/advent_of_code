...
(1) 3  0  1  -3  - jump with offset 0 (that is, don't jump at all). Fortunately, the instruction is then incremented to
1.
...
my_jumps = pd.read_csv('05_input.csv', header=None).values.flatten().tolist()


def calculate_steps(jump_list, reset=None):

    # make sure we are not editing the original list inplace
    new_jumps = deepcopy(jump_list)
    i = 0
    step_count = 0
    ...
    while True:
        ...
        # get offset from list and update list index with the offset
        offset = new_jumps[i]
        new_i = offset + i

        # conditionally increment or decrement offset
        if reset is not None and offset >= reset:
            new_jumps[i] -= 1
        else:
            new_jumps[i] += 1
        ...
        # return step count once new_i is outside of the original list
        if new_i >= len(jump_list):# or new_i < 0:
            return step_count
...
if __name__ == '__main__':
    ...
    # ------ Part 1 ------ #
    assert calculate_steps([0, 3, 0, 1, -3]) == 5
    # display my solution
    print('Part 1: ', calculate_steps(my_jumps))

    # ------ Part 2 ------ #
    assert calculate_steps([0, 3, 0, 1, -3], reset=3) == 10
    # display my solution
    print('Part 2: ', calculate_steps(my_jumps, reset=3))
